...
import { Loader2, ArrowLeft, MailWarning } from 'lucide-react'
import Link from 'next/link'
import { ThemeToggle } from '@/components/ThemeToggle'
import { BrandMark } from '@/components/BrandMark'

type Props = {
  /** El enlace del email venció o ya se usó — lo manda /auth/confirm. */
  enlaceInvalido?: boolean
}

export function LoginClient({ enlaceInvalido = false }: Props) {
    ...
    <div className="relative flex flex-1 items-center justify-center min-h-screen bg-noche overflow-hidden px-4 py-10">
      ...
      <div className="relative z-10 w-full max-w-[420px]">
        <Card className="bg-card border-none rounded-2xl shadow-2xl shadow-black/30">
          ...
          <CardContent>
            {/* Quien llega acá desde un enlace vencido no tiene contraseña todavía: sin este
                cartel, el formulario de abajo le pide algo que nunca creó y no hay nada en
                pantalla que explique qué pasó ni cómo salir. */}
            {enlaceInvalido && (
              <div role="status" className="mb-6 rounded-xl border border-marca/25 bg-marca/5 p-4">
                <p className="flex items-center gap-2 font-sans text-sm font-semibold text-tinta">
                  <MailWarning className="h-4 w-4 shrink-0 text-marca" aria-hidden />
                  El enlace del email ya no sirve
                </p>
                <p className="mt-2 font-serif text-sm leading-relaxed text-muted-foreground">
                  Los enlaces de acceso vencen al poco tiempo por seguridad, y también dejan de
                  funcionar si ya se usaron. Pedí uno nuevo y vas a poder crear tu contraseña.
                </p>
                <Link
                  href="/recuperar-contrasena"
                  className="mt-3 inline-block font-sans text-sm font-semibold text-marca underline underline-offset-2 hover:no-underline"
                >
            ...
            <form action={handleSubmit} className="mt-4">
              ...
              <div className="mt-2 text-right">
                {/* El texto cubre los dos casos a propósito: la misma pantalla la usa quien
                    perdió su contraseña y quien nunca llegó a crearla porque su invitación
                    venció. Los dos salen por acá. */}
