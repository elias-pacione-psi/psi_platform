...
import { actualizarPaciente, cambiarEstadoPaciente, eliminarUsuarioTotal } from '../actions'

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function UserActionsCell({ paciente, todosLosProgramas, stateType }: { paciente: any, todosLosProgramas: any[], stateType: string }) {
  ...
  const [selectedProgramas, setSelectedProgramas] = useState<string[]>(paciente.programasSeleccionados || [])
    ...
    startTransition(async () => {
      try {
        ...
        if (result?.error) {
          ...
          toast.success("Perfil de paciente actualizado")
        }
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      } catch (err: any) {
  ...
  const handleEstado = async (id: string, estado: 'activo' | 'suspendido' | 'eliminado') => {
    startTransition(async () => {
      try {
        const result = await cambiarEstadoPaciente(id, estado)
        if (result.error) {
          toast.error(`Error al cambiar estado: ${result.error}`)
        } else {
          toast.success(`Estado del paciente actualizado a ${estado}`)
        }
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      } catch (err: any) {
  ...
  const executeArchive = () => {
    handleEstado(paciente.id, 'eliminado')
  ...
  return (
    <>
            ...
            <>
              {stateType === 'activo' ? (
                <DropdownMenuItem onClick={(e) => { e.preventDefault(); handleEstado(paciente.id, 'suspendido'); }}>
                  ...
                  <span className="text-orange-600">Suspender acceso</span>
                </DropdownMenuItem>
              ) : (
                <DropdownMenuItem onClick={(e) => { e.preventDefault(); handleEstado(paciente.id, 'activo'); }}>
                  <CheckCircle2 className="mr-2 h-4 w-4 text-green-600" />
                  <span className="text-green-600">Reactivar acceso</span>
                </DropdownMenuItem>
              )}
              ...
              <DropdownMenuItem onClick={(e) => { e.preventDefault(); setShowDeleteDialog(true); }}>
                ...
                <span className="text-red-600 font-bold">Eliminar / Archivar</span>
              </DropdownMenuItem>
            </>
          )}
          ...
          {stateType === 'eliminado' && (
            <>
              <DropdownMenuItem onClick={(e) => { e.preventDefault(); handleEstado(paciente.id, 'activo'); }}>
                ...
                <span className="text-green-600">Restaurar usuario</span>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={(e) => { e.preventDefault(); setPurgeText(''); setShowPurgeDialog(true); }}>
                ...
                <span className="text-red-700 font-bold">Eliminar definitivamente</span>
              </DropdownMenuItem>
            </>
          )}
        </DropdownMenuContent>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="font-sans">
              Esto bloquea el acceso del paciente a la plataforma (ban) y lo mueve al historial, conservando su cuenta y asignaciones por si vuelve a retomar.
      ...
      <AlertDialog open={showPurgeDialog} onOpenChange={(open) => { setShowPurgeDialog(open); if (!open) setPurgeText('') }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="text-red-700 font-serif text-2xl">Eliminar definitivamente</AlertDialogTitle>
            <AlertDialogDescription className="font-sans">
              Esto borra de forma <strong>irreversible</strong> a <strong>{paciente.nombre}</strong> y todos sus datos: cuenta, asignaciones de contenido y agenda de sesiones. No se puede deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
          ...
          <form action={handleSubmitEdit} className="space-y-6 mt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input id={`nombre-${paciente.id}`} name="nombre" defaultValue={paciente.nombre} className="bg-white border-gray-200" required />
              </div>
              <div className="space-y-2">
                <Label htmlFor={`email-${paciente.id}`} className="font-bold text-tinta">Correo electrónico</Label>
                <Input id={`email-${paciente.id}`} defaultValue={paciente.email} className="bg-gray-100 border-gray-200 text-gray-500 cursor-not-allowed" disabled />
                <p className="text-xs text-gray-400 mt-1">El email no puede cambiarse acá por seguridad.</p>
              </div>
              <div className="space-y-2">
                <Label htmlFor={`telefono-${paciente.id}`} className="font-bold text-tinta">Teléfono / WhatsApp</Label>
                <Input id={`telefono-${paciente.id}`} name="telefono" defaultValue={paciente.telefono || ''} className="bg-white border-gray-200" />
              </div>
              <div className="space-y-2">
                <Label htmlFor={`link_videollamada-${paciente.id}`} className="font-bold text-tinta">Link de videollamada</Label>
                <Input id={`link_videollamada-${paciente.id}`} name="link_videollamada" defaultValue={paciente.link_videollamada || ''} className="bg-white border-gray-200" placeholder="https://meet.google.com/... o Zoom" />
              </div>
            </div>
              ...
              <div className="grid gap-3 max-h-[200px] overflow-y-auto p-2 bg-white rounded-xl border border-gray-200">
                ...
                {todosLosProgramas.map((programa) => (
                  <div key={programa.id} className="flex items-center space-x-3 bg-gray-50 p-3 rounded-lg border border-gray-100">
                    <Checkbox
                      id={`programa-${paciente.id}-${programa.id}`}
